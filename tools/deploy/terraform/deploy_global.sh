...
terraform plan -var-file=common.tfvars -var-file=global.tfvars -input=false -out=$TMP_PLAN -detailed-exitcode > $TMP_PLAN_STDOUT

plan_ec=$?

if [ $plan_ec -eq 0 ]; then
  echo "No changes for global state"
...
terraform apply -var-file=common.tfvars -var-file=global.tfvars
