...
def main():
    ...
    parser.add_argument(
        ...
        dest='input',
        required=True,
        help='Input .csv file. Should be the output of the granulesToSams.py script'
    )

    parser.add_argument(
        '-o', '--output',
        dest='output',
        default=os.path.join(os.getcwd(), 'oco3-sam-targets-aggregated-bounding-boxes.csv'),
    ...
    agg_df = df.groupby('Target ID').agg(
        {'Target Name': 'first', 'min_lon': 'min', 'min_lat': 'min', 'max_lon': 'max', 'max_lat': 'max'})
    ...
    if args.geo and has_gp:
        try:
            ...
            gdf = geopandas.GeoDataFrame(agg_df, geometry='Bounding Box WKT')
